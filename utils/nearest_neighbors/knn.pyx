...
cdef extern from "knn_.h":
    ...
    void cpp_query_from_radius(const float* points, const size_t npts, const size_t dim,const float* queries, const size_t nqueries,
...
def knn_batch_distance_pick(pts, nqueries, K, omp=False):
    ...
    # create indices tensor
    indices = np.zeros((pts.shape[0], nqueries, K), dtype=np.longlong)
...
def query_from_radius(pts, nqueries, radius, MAX_COUNT = 1000):
    # pts [n,3] , nqueries [m,3]
    # MAX_COUNT are max neibours points count
    # define shape parameters
    cdef int npts
    cdef int nqueries_cpp
    cdef int radius_cpp
